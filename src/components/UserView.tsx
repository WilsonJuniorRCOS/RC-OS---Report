...
import { Send, Image as ImageIcon, CheckCircle2, AlertTriangle, ExternalLink, X, Clock, RefreshCw, Trash2, Eye } from 'lucide-react';
...
export const UserView: React.FC<UserViewProps> = ({
  ...
  const handleDeleteReportClick = async (reportId: string, title: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const ok = await onSubmitReport({
        ...
        autor_nome: user.nome || user.email || 'Consultor',
      });
      ...
      if (ok) {
        ...
        alert('Não foi possível registrar o report. Por favor, tente novamente.');
      }
    } catch (err: any) {
              ...
              {reports.map((report) => (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                    ...
                    {/* Status Highlight Badge & Delete button */}
                    <div className="flex items-center gap-2">
                      {getStatusBadge(report.status)}
                      {onDeleteReport && (
                        <button
                          type="button"
                          onClick={() => handleDeleteReportClick(report.id, report.titulo)}
                          disabled={deletingId === report.id}
                          title="Excluir este report"
                  ...
                  {report.print_url && (
                    <div className="mt-2 p-2 bg-slate-950/90 rounded-xl border border-slate-200 flex items-center gap-3">
                      <img
                        src={report.print_url}
                        alt="Thumbnail"
                        className="w-16 h-16 object-cover rounded-lg cursor-pointer hover:opacity-90 transition-opacity shrink-0"
                        onClick={() => onOpenDetailModal ? onOpenDetailModal(report) : onOpenImageModal && onOpenImageModal(report.print_url!)}
                      />
                      <div className="flex-1 min-w-0">
                        <span className="text-[11px] font-bold text-slate-300 block">Print Anexado</span>
                        <button
                          type="button"
                          onClick={() => onOpenDetailModal ? onOpenDetailModal(report) : onOpenImageModal && onOpenImageModal(report.print_url!)}
                          className="mt-1 text-xs text-amber-400 hover:underline font-semibold flex items-center gap-1 cursor-pointer"
                        >
                          <ImageIcon className="w-3.5 h-3.5" /> Clique para ver print completo
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="pt-2 flex flex-wrap items-center justify-between gap-2 text-xs border-t border-slate-100">
                    ...
                    >
                      <Eye className="w-4 h-4 text-amber-400" /> Ver na Íntegra & Print
                    </button>
                    ...
                    <div className="flex items-center gap-2">
                      {report.link && (
                        <a
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold rounded-lg text-xs transition-colors cursor-pointer"
                        >
